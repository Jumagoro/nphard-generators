...
class TestHCPSynH2Factory:
    ...
    def test_basic_requirements(self):
        ...
        assert isinstance(synh2_problem, HCProblemSolution)
        assert synh2_problem.is_hamiltonian
        ...
        assert synh2_problem.graph_density == pytest.approx(density, abs=0.15),"density incorrect"

        assert synh2_problem.n_edges == pytest.approx(
            n_edges_should, abs=n_edges_should*0.15),"n_edges incorrect"

        for (i, _) in enumerate(synh2_problem.cycle_nodes):  # Check if cycle is complete
            node_a = synh2_problem.cycle_nodes[i-1]
            node_b = synh2_problem.cycle_nodes[i]
            assert synh2_problem.has_edge(node_a, node_b)
